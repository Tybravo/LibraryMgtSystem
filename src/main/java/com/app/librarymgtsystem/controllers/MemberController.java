...
public class MemberController {
    ...
    @PatchMapping("/update-session-status")
    public ResponseEntity<String> updateSessionStatus(@RequestBody Map<String, String> requestBody) {
        String email = requestBody.get("email");
        if (email == null || email.isEmpty()) {
            return ResponseEntity.badRequest().body("Email is required.");
        }

        boolean isUpdated = memberService.updateSessionStatus(email);
        if (isUpdated) {
            return ResponseEntity.ok("Session status updated successfully.");
        } else {
